...
namespace MessagePipe.SharedMemory.InternalClasses
{
    ...
    public sealed class QueueOptions
    {
        ...
        public QueueOptions(string queueName, long capacity)
            : this(queueName, System.IO.Path.GetTempPath(), capacity)
        {
        }
        ...
        {
            QueueName = Contract.CheckNonEmpty(queueName, nameof(queueName));
            Path = Contract.CheckValue(path, nameof(path));

            Capacity = Contract.CheckGreaterThan(capacity, 16, nameof(capacity));
            Contract.CheckParam((capacity % 8) == 0, nameof(queueName), "messageCapacityInBytes should be a multiple of 8 (8 bytes = 64 bits).");
        }
